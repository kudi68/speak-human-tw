...
不想要這關的話，在指令裡明講「不用列清單，直接幫我改」就會跳過。**非互動情境**（`codex exec`、CI、排程）也要在當次指令或 workflow 設定裡明確選擇直接套用；未選模式時只產生標示「未套用」的完整建議清單，不會因為沒有人回覆就改掉原稿。
...
這是刻意的，因為你的原稿比它的建議值錢。完整流程見上面的[〈它不會直接改你的稿〉](#第一次用會遇到的事它不會直接改你的稿)。不想要這關就說「不用列清單，直接幫我改」；非互動情境也必須在 prompt 或 workflow 設定中明確選定直接改寫，沒有選定就只交付未套用的建議清單。
