...
changelog:
  - "Unreleased (2026-09-07): 修正非互動權限邊界。沒有人能回覆不再等於全部同意；只有當次 prompt 或既有 workflow 設定已明確授權直接改寫時才套用，否則只輸出可供審查的完整建議清單，不改原稿。"
...
- **非互動環境**：沒有後續對話輪次不構成同意。只有當次 prompt 明確要求「直接套用／直接改檔」，或既有 workflow 設定已明確選定「跳過確認、事後摘要」，才可直接改寫。沒有這項授權時，輸出完整建議清單作為 review artifact，清楚標示「未套用」，不改原稿，也不把沉默視為全選。
- 這個 skill 要被**長期整合**進自動化工作流（排程、CI、內容產線）→ 見下方「自動化工作流模式」，接進去之前要先問使用者選哪一種模式，不能自己假設。

### 自動化工作流模式：整合進 pipeline 時

當這個 skill 要被接進自動化工作流，**在建立或設定 workflow 時先取得一次明確選擇**，不要在執行時從沉默、逾時或無人值守推定模式：

- **保留確認清單**：工作流跑到這一步照樣暫停，照上面「強制規則」列清單、等人工回覆選項。適合還在調整改寫品質、或處理的是會直接對外發布的內容。
- **跳過確認、事後摘要**：只有 workflow 設定已明確選定此模式時，才不停下來等人，直接把步驟 1–5 分析出的建議全部套用；跑完後輸出摘要供事後檢查，也方便用 `git diff` 回溯。適合已經信任這個 skill 的判斷品質、追求自動化效率、後面還有其他人工或系統關卡把關的情境。

若非互動工作流沒有保存任何模式選擇，預設為**建議清單、未套用**：完成分析並交付可審查結果，不提出無人能回答的問句，也不修改來源。外部發布、寄送或覆蓋正式內容仍須有相應授權；選定直接改寫模式不自動授權這些後續動作。
...
- **先列編號清單（原句／為什麼要改／建議怎麼改），取得明確授權後才動筆或動檔案**——互動情境依上方問句等回覆；非互動情境依已保存的 workflow 模式或當次 prompt，未授權時輸出「未套用」清單。
...
- **預設輸出＝「強制規則」的第一輪編號清單，不是改寫版。** 互動情境清單問完就等回覆；非互動情境標示「未套用」並正常結束，不修改來源。
